...
class EarningsValidationException implements Exception {
  final List<String> errors;

  EarningsValidationException(this.errors);

  @override
  String toString() => 'EarningsValidationException: ${errors.join("; ")}';
}
...
/// Earnings Report API client.
///
/// For more information, visit: https://apiverve.com/marketplace/earnings?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [ticker] (required) - Stock ticker symbol (e.g. AAPL, MSFT, ADBE) [minLength: 1, maxLength: 5]
/// * [year] - Fiscal year to retrieve. Defaults to latest available. [min: 2000, max: 2030]
/// * [quarter] - Fiscal quarter to retrieve. Defaults to latest available. [min: 1, max: 4]
class EarningsClient {
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'ticker': ValidationRule(type: 'string', required: true, minLength: 1, maxLength: 5),
    'year': ValidationRule(type: 'integer', required: false, min: 2000, max: 2030),
    'quarter': ValidationRule(type: 'integer', required: false, min: 1, max: 4),
  };

  EarningsClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/earnings',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [EarningsValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw EarningsValidationException(errors);
    }
  }

  /// Execute a request to the Earnings Report API.
  ///
  /// Parameters are validated before sending the request.
  Future<EarningsResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw EarningsException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return EarningsResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw EarningsException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw EarningsException('Resource not found');
      } else {
        throw EarningsException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is EarningsException) rethrow;
      throw EarningsException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Earnings Report API client.
class EarningsException implements Exception {
  final String message;

  EarningsException(this.message);

  @override
  String toString() => 'EarningsException: $message';
}
