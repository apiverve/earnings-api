...
class EarningsAPIClientError(Exception):
...
class ValidationError(EarningsAPIClientError):
...
class EarningsAPIClient:
    ...
    VALIDATION_RULES = {"ticker": {"type": "string", "required": True, "minLength": 1, "maxLength": 5}, "year": {"type": "integer", "required": False, "min": 2000, "max": 2030}, "quarter": {"type": "integer", "required": False, "min": 1, "max": 4}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the EarningsAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises EarningsAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/earnings'
        self.headers = {
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[EarningsAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns EarningsReportData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises EarningsAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise EarningsAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise EarningsAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise EarningsAPIClientError(f"Request failed: {str(e)}")
